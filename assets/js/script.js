...
let splashTitleArea = document.getElementById('splash-title-area')
let splashShader = document.getElementById('splash-shader')
...
function initializeValues() {
  ...
  setTimeout(() => {
    ...
    mainArea.style.border = '.2em solid var(--acnt-f)'
...
mainArea.addEventListener('click', () => {
  ...
  if (eventTarget.className === 'input-element-I' || eventTarget.className === 'input-element-II') {
    ...
    if (eventTargetGradientValues[0] === rotatorGradientValues[0] && eventTargetGradientValues[1] === rotatorGradientValues[1]) {
      ...
      eventTarget.style.border = '.2em solid var(--clr)'
...
function areaWellFlow(flowSpeedValue) {
  ...
  wellFlowInterval = setInterval(() => {
    if (count > 100) {
      ...
      // overflow anim
      mainArea.style.border = '.2em solid var(--invalid-selection)'
...
// splash in
let titleValue = 'Duosplash'
for (let x = 0; x < titleValue.length; x++) {
  let titleNode = document.createElement('div')
  titleNode.setAttribute('class', 'splash-title-item')
  titleNode.innerText = `${titleValue[x]}`
  splashTitleArea.appendChild(titleNode)
}

let splashTitleItems = document.querySelectorAll('.splash-title-item')
splashShader.addEventListener('click', () => {
  let titleCounterA = 3
  let titleCounterB = 5
  splashTitleItems[4].style.opacity = '0'
  let titleCountInterval = setInterval(()=> {
    if (titleCounterA < 0) {
      setTimeout(()=> {
	splashShader.style.opacity = '0'
	setTimeout(()=> {
	  splashShader.style.display = 'none'
	  clearInterval(titleCountInterval)
	}, 1100)
      }, 100)
    } else {
      splashTitleItems[titleCounterA].style.opacity = '0'
      splashTitleItems[titleCounterB].style.opacity = '0'
      titleCounterA -= 1
      titleCounterB += 1
    }
  }, 200)
...
elementItems = [
  ...
  mainArea, displayPanel,
  menu
]
let loadIndex = 0
setTimeout(()=> {
  let loadInterval = setInterval(()=> {
    if (loadIndex === elementItems.length) {
      clearInterval(loadInterval)
    } else {
      elementItems[loadIndex].style.opacity = '100'
      loadIndex += 1
    }
  }, 200)
}, 1700)
